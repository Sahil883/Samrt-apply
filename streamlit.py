...
def main():
    ...
    if 'job_data' in st.session_state and not st.session_state.job_data.empty:
        display_results(st.session_state.job_data, st.session_state.search_params)
...
def display_results(df, search_params, is_sample=False):
...
def display_job_card(job):
    ...
    source_class = "linkedin-badge" if job['source'] == 'LinkedIn' else "naukri-badge"
    ...
    <div class="job-card">
        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 1rem;">
            ...
            <span class="source-badge {source_class}">{job['source']}</span>
        </div>
